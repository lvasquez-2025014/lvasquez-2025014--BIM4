  ...
  async loginWithGoogle(idToken?: string) {
    ...
    try {
      ...
      try {
        ...
        console.warn('[NestJS Auth] Verificación local falló (posible desfase horario), consultando endpoint oficial de Google tokeninfo...', verifyErr?.message);
        ...
        if (data.aud !== this.googleClientId) {
          throw new UnauthorizedException('Audiencia de token de Google no coincide');
        }
        payload = {
          email: data.email,
          name: data.name,
